...
PsaSwitch::receive_(port_t port_num, const char *buffer, int len) {
  ...
  std::cout << "recv -> received packet \n";

  if (do_swap() == 0) {
    check_queueing_metadata();
  }

  std::cout << "recv -> past queueing meta check \n";
  ...
  std::cout << "recv -> have packet ptr now \n";

  BMELOG(packet_in, *packet);

  std::cout << "recv -> getting phv now \n";
  PHV *phv = packet->get_phv();
  // many current P4 programs assume this
  // it is also part of the original P4 spec
  std::cout << "recv -> resetting meta now \n";
  phv->reset_metadata();

  // setting standard metadata

  // phv->get_field("standard_metadata.ingress_port").set(port_num);
  std::cout << "recv -> SENDING TO INPUT BUF NOW \n";
...
PsaSwitch::enqueue(port_t egress_port, std::unique_ptr<Packet> &&packet) {
    std::cout << "ENQ -> checking if i exceed maxport: " << max_port << "\n";

    if (egress_port >= max_port) {
      bm::Logger::get()->error("Invalid egress port %u, dropping packet",
                               egress_port);
      std::cout << "ENQ -> turns out i exceeded maxport\n";
      return;
    }

    std::cout << "ENQ -> setting egress port\n";
    ...
    std::cout << "ENQ -> pushing to egress buffer w/ priority\n";
    egress_buffers.push_front(
        egress_port, SSWITCH_PRIORITY_QUEUEING_NB_QUEUES - 1 - priority,
        std::move(packet));
#else
    std::cout << "ENQ -> pushing to egress buffer\n";
...
PsaSwitch::ingress_thread() {
  ...
  while (1) {
    ...
    std::cout << "INGRES THREAD -> have packet ptr now \n";
    ...
    //   continue;
    // }
    std::cout << "INGRES THREAD -> moving packet to egress buffer \n";
    port_t egress_port = 69;
    enqueue(egress_port, std::move(packet));
    std::cout << "INGRES THREAD -> made it out of enqueue()\n";
  }
}
...
PsaSwitch::egress_thread(size_t worker_id) {
  ...
  while (1) {
    ...
    std::cout << "EGRESS THREAD -> getting deparsers\n";

    Deparser *deparser = this->get_deparser("egress_deparser");

    std::cout << "EGRESS THREAD -> getting egress pipeline\n";

    Pipeline *egress_mau = this->get_pipeline("egress");

    std::cout << "EGRESS THREAD -> getting phv\n";
